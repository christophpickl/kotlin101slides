
\sektion{Introduction}

\frame{\frametitle{Hard Facts} 
  \begin{itemize}
    \item Statically typed, hybrid programming language for the JVM
    \item 100\% interoperable with Java (1.6)
    \item Runs on Android, compiles to JavaScript
    \item Focuses on industry, tooling and safety
    \item Open sourced compiler and tools (Apache 2 license)
  \end{itemize}
}

\frame{\frametitle{Background} 
  \begin{itemize}
    \item Development started 2010
    \item \href{https://blog.jetbrains.com/kotlin/2016/02/kotlin-1-0-released-pragmatic-language-for-jvm-and-android/}{Version 1.0} Released February, 2016
    \item Developed by JetBrains (IntelliJ, ReSharper, \ldots)
  \end{itemize}

\vspace{10.0pt}

\begin{figure}[h]
\centering
\includegraphics[height=2.0cm]{andrey}
\vspace{-10.0pt}
\caption{Andrey Breslav}
\end{figure}
}


\frame{\frametitle{Yet another JVM language}
  \begin{itemize}
    \item Nothing new, just the best of all of them
    \item Well known company with good tooling support
    \item Scala for the masses
  \end{itemize}
}